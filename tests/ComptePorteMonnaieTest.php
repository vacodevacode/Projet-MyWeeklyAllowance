...
/** ---- Historique des transactions | Fonction fait par Fouad ---- */   
...
/** ---- Allocation Hebdomadaire | Fonction fait par Danali ---- */

    public function AllocationHebdomadaire(): void 
    {
        
        $ComptePorteMonnaie = new ComptePorteMonnaie("Gabrielle");
        $ComptePorteMonnaie->AppliquerAllocationHebdomadaire(20);

        $ComptePorteMonnaie->recevoirAllocationHebdomadaire();

        
        $balance = $ComptePorteMonnaie->getBalance();
        
        $this->assertSame(20, $balance, 'Le solde doit être de 20€ après réception de l\'allocation hebdomadaire.');

        $historique = $ComptePorteMonnaie->getHistoriqueTransactions();
        $this->assertSame('Allocation Hebdomadaire', $historique[0]['type'], 'La transaction doit être un dépot de l\'allocation hebdomadaire.');
        $this->assertSame(20, $historique[0]['montant'], 'La transaction doit être un dépot de 20€.');
    }

    public function testAllocationHebdomadaireNegativeLanceUneException() 
    {
        $ComptePorteMonnaie = new ComptePorteMonnaie("Gabrielle");

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage("L'allocation hebdomadaire doit être un montant positif.");
        
        $ComptePorteMonnaie->DefinirAllocationHebdomadaire(-10);
    }
